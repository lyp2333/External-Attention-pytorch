...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.shift_size = shift_size  # window_shift的patch数
        self.mlp_ratio = mlp_ratio  # 多层感知机的放大比例
        if min(self.input_resolution) <= self.window_size:
            # 输入图像的分辨率小于window的大小时，我们不分割窗口
            self.shift_size = 0
            self.window_size = min(self.input_resolution)

        assert 0 <= self.shift_size < self.window_size, f"shift_size must in (0,{self.window_size})"
    ...
    def forward(self, input):
        H, W = self.input_resolution
        B, L, C = input.shape
        assert L == H * W, f"input feature应该是({H},{W})"
        ...
        # 此时attn_windows.shape = num_windows,window_size,window_size,C
        shifted_x = window_reverse(attn_windows, self.window_size, H, W)  # B H' W' C

        # 此时得到的仍是shift过的patch，要将其shift回去
        if self.shift_size > 0:
